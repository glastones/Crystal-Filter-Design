...
uses
  ...
  ExtCtrls, Grids,IniFiles,Math,simFunctions,Graph_Result;
...
procedure TForm3.btnResultClick(Sender: TObject);
var
cheby:boolean;
...
begin
  ...
  AssignFile(f,'4_Pole_Response.csv');
try
  ReWrite(f);
  try
  writeln(f,'Measurement,Frequency,Gain');
   ...
   on E:Exception do
     ShowMessage('File 4_pole_Response.csv could not be read or written because: '+E.ClassName+' / '+ E.Message);
      end;
...
if cheby=True then
    Temp_Result:= ChebyBPGain(Counter,fl,fu,4,Ripple) {calculate response}
Else
Temp_Result:= ButterBandpassGain(Counter,fL,fU,4);
