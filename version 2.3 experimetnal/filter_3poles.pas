...
uses
  ...
  Grids, Math, simFunctions, IniFiles,Graph_Result;

type

  { TFiter_3pole }

  TFiter_3pole = class(TForm)
    btnExit: TButton;
    btnResult: TButton;
    ...
    procedure txtCrystal_1_FrequencyChange(Sender: TObject);
    procedure txtCrystal_2_Frequency1Change(Sender: TObject);
  private
...
procedure TFiter_3pole.btnExitClick(Sender: TObject);
begin
  Close;
end;

procedure TFiter_3pole.btnResultClick(Sender: TObject);
   var
   cheby:boolean;
   ...
   begin
     ...
     AssignFile(f,'3_Pole_Response.csv');
   try
     ReWrite(f);
     try
     writeln(f,'Measurement,Frequency,Gain');
      ...
      on E:Exception do
        ShowMessage('File 3_pole_Response.csv could not be read or written because: '+E.ClassName+' / '+ E.Message);
         end;
   ...
   if cheby=True then
       Temp_Result:= ChebyBPGain(Counter,fl,fu,3,Ripple) {calculate response}
   Else
   Temp_Result:= ButterBandpassGain(Counter,fL,fU,3);
...
procedure TFiter_3pole.FormCreate(Sender: TObject);
begin
   txtSimCF.Text:='';
  txtSimSpan.Text:='';
  txtSimStep.Text:='';
...
procedure TFiter_3pole.FormShow(Sender: TObject);
   var
   INI : TINIFile;
...
procedure TFiter_3pole.txtCrystal_1_FrequencyChange(Sender: TObject);
begin

end;

procedure TFiter_3pole.txtCrystal_2_Frequency1Change(Sender: TObject);
begin
